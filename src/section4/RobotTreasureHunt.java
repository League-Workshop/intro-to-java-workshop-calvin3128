...
public class RobotTreasureHunt implements KeyEventDispatcher{
	...
	private void spaceBarWasPressed() {
		...
		int robotXLocation = ferb.getX();
		int robotYLocation = ferb.getY();
		...
		if(robotXLocation <= 750 && robotXLocation >= 700 && robotYLocation >= 380 && robotYLocation <= 480) {
		...
		if(robotXLocation <= 250 && robotXLocation >= 160 && robotYLocation >= 20 && robotYLocation <= 90) {
			JOptionPane.showMessageDialog(null, "Voyage to the land of the keep out sign.");
		}
		
		if(robotXLocation <= 900 && robotXLocation >= 770 && robotYLocation >= 220 && robotYLocation <= 340) {
			JOptionPane.showMessageDialog(null, "The treasure lies within flames.");
		}
		
		if(robotXLocation <= 90 && robotXLocation >= 60 && robotYLocation >= 250 && robotYLocation <= 280) {
			treasureFound();
		}
